...
if(isset($_POST['nome'])){
	...
	require_once 'model/vermifugo.php';
	$resposta=alterarVermifugo($Id_verm,$nome,$marca,$lote,$fabricacao,$validade,$aplicacao,$proximaapli,$Identificacao_vac);
	if (!$resposta) {
...
<form action="alterarVermifugo.php" method="POST">  
	...
	<p>Proxima aplicação:<input type="date" name="proximaapli" min='2000-01-01' max='2023-03-12'></p>
		...
		<input type="hidden" name="Id_verm" value="<?php echo $linha['Id_ver']; ?>">
...
$nomeVer=$_POST['nomeVer'];
	...
	require_once 'model/vermifugo.php';
	$resposta=alterarVermifugo($Id_verm,$nome_cav,$marca,$lote,$fabricacao,$validade,$aplicacao,$proximaapli,$Identificacao_cav);
	if (!$resposta) {
			echo "<h2>Erro na tentativa de alterar!</h2>";
		}else{
			echo "<h2>Alterado com sucesso!</h2>";
